...
class BrainEngine:
    ...
    def guardar_contexto(self, texto: str, intent_data: dict):
        ...
        if entities.get("app_name"):
            dados_contexto["last_apple_tv_app"] = entities.get("app_name")
    ...
    def resolver_followup(self, texto: str) -> str:
        ...
        if self.ultimo_intent and self.ultimo_intent.startswith("media.apple_tv_"):
            if texto_lower.startswith("e "):
                resto = texto_lower[2:].strip()
                return f"{resto} na apple tv"

            if texto_lower.startswith("pausa"):
                return "pausa a apple tv"

            if texto_lower.startswith("continua") or texto_lower.startswith("play"):
                return "continua a apple tv"

            if texto_lower.startswith("menu"):
                return "menu na apple tv"

            if texto_lower.startswith("home"):
                return "home na apple tv"

            if texto_lower.startswith("abre ") or texto_lower.startswith("abdo "):
                return f"{texto_lower.replace('abdo ', 'abre ', 1)} na apple tv"
